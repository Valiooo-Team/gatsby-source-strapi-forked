...
const fetchEntity = async ({
  ...
  try {
    ...
    return (0, _lodash.castArray)([data.data, ...otherLocalizationsData]).map(entry => (0, _cleanData.cleanData)(entry, {
      ...ctx,
      contentTypeUid: uid
    }));
  } catch (error) {
...
const fetchEntities = async ({
  ...
  try {
    ...
    const fetchPagesPromises = pagesToGet.map(page => async () => {
      const options = {
        ...opts
      };
    ...
    const results = [];
    const chunkSize = 3;
    for (let i = 0; i < fetchPagesPromises.length; i += chunkSize) {
      results.push(await Promise.all(fetchPagesPromises.slice(i, i + chunkSize).map(x => x())));
    }

    // const fetchPagesPromises2 = async () => {
    //   const allData = [];

    //   reporter.info(`Total pages - ${pagesToGet.length}`);

    //   for (let i = 0; i < pagesToGet.length; i++) {
    //     const page = pagesToGet[i];
    //     const options = { ...opts };

    //     options.params.pagination.page = page;
    //     reporter.info(`${i} - Starting to fetch data from Strapi - ${options.url} with ${JSON.stringify(opts.paramsSerializer(opts.params))}`);
    ...
    // const results = await Promise.all(fetchPagesPromises2);
    // const results = await fetchPagesPromises2();
    const cleanedData = [...data, ...(0, _lodash.flattenDeep)(results)].map(entry => (0, _cleanData.cleanData)(entry, {
      ...ctx,
      contentTypeUid: uid
    }));
    return cleanedData;
...
function chunck(arrays, chunkSize) {
  const chunks = [];
  for (let i = 0; i < arrays.length; i += chunkSize) {
    chunks.push(arrays.slice(i, i + chunkSize));
  }
